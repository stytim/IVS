...
public class KokoroTTSSpeaker : MonoBehaviour, ILocalTTSSpeaker
{
    ...
    public HeadTurn headTurn; // Head turn script reference
    ...
    public void SpeakDelayed()
    {
        if (!isPlaying && !llmHandler.isSelf)
        {
            // if audio is not generated yet, we wait till audioQueue.Count > 0
            // and then we start the coroutine
            StartCoroutine(PlayQueuedAudio());
        }
    ...
    private IEnumerator PlayQueuedAudio()
    {
        ...
        MessageHandler.Instance?.SendMessageToNetwork("ttsd");
        llmHandler.TriggerSubtitleFadeOut();
        if (llmHandler.isAsking)
        {
            headTurn.WaitForResponse();
            llmHandler.isAsking = false;
        }
    }
