...
public class MessageHandler : MonoBehaviour
{
    ...
    void Update()
    {
        while (messageQueue.TryDequeue(out string message))
        {
            ...
            llmHandler.PatientQuestion(message);
        }

        while (actionQueue.TryDequeue(out string message))
        {
            if (llmHandler.isReceived)
            {
                headTurn.ReceivedResponse();
                llmHandler.isReceived = false;
            }
            else
            {
    ...
    private void HandleConnection(TcpClient tcpClient)
    {
        ...
        try
        {
            ...
            while (isRunning && tcpClient.Connected && isConnected) // Added isConnected here too
            {
                ...
                else if (message == "ttsp") // Or your other TTS command
                {
