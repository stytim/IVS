...
public class STTHandler : MonoBehaviour
{
    ...
    private void HandleTranslationResult(string translatedText)
    {
        ...
        llmhandler.DoctorQuestion("Doctor:" + translatedText);
    ...
    private void HandleFinalTranscription(string transcribedText)
    {
        ...
        if (llmhandler != null)
        {
            ...
            {
                // Check if the transcribed text starts with "Alex"
                //if (!transcribedText.Contains("Alex", StringComparison.OrdinalIgnoreCase))
                //    return; // Ignore if it doesn't start with "Alex"

                //llmhandler.DoctorQuestion("Doctor: " + transcribedText);
